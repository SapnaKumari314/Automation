...
public class TOUC2828_ReassignmentMaintenanceOutsideGarageToNonManitenance extends BaseTestClass{
	...
	@Test(
			description="TOUC-2828: (CCSS):Re-assignment: Validate user is able to re-assign manitenance bus which is not in the garage as Non Maintenance to run and track",
	...
	public void Test_TOUC_2828() throws InterruptedException, IOException {
		...
		expectedMessageForAssignment = "Success: Bus "+busName+" has been successfully set In Maintenance but not assigned to a position in the garage";
		...
		sAssert.assertEquals(rrtPage.CCSS_TrackDetailsComponentHeader.getText(), "Recommended Track", "Track Details component header after searching assigned bus");
		sAssert.assertTrue(uiTrackDetailsMapForAssignment.get(rrtPage.TRACK_NO).isEmpty(), "Track details are not empty");
		...
		sAssert.assertNotEquals("", trackPositionForReAssignment, "Track Position for assignment and re-assignment");
