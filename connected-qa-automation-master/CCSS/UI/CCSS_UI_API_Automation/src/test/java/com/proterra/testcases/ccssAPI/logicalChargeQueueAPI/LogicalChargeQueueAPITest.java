package test.java.com.proterra.testcases.ccssAPI.logicalChargeQueueAPI;
...
public class LogicalChargeQueueAPITest extends APIBaseTest implements Modules{

	//Initiate Logger
	public static Logger log = Logger.getLogger(LogicalChargeQueueAPITest.class.getName());
	...
	@Test(priority = 0, description = "TOUC-3263: (CCSS.API): Logical Charge Queue: Verify logicalChargeQueue API with valid data and response")
	public void CCSS_API_LogicalChargeQueueValidData() {
		...
		//Set path
		String path = getPATH_LOGICALCHARGEQUEUE(TENANT_ID, GARAGE_ID);
		...
		//Add Query Params
		request.queryParam("requestId", "GET_LOGICAL_CHARGE_QUEUE-"+generateUUID());
		...
		sAssert.assertEquals(status, 200, "Logical Charge Queue API - Status Code for Valid user");
		sAssert.assertEquals(bool, false, "Logical Charge Queue API - Response Body is not empty for Valid user");
		sAssert.assertAll();
	...
	@Test(priority = 1, description = "TOUC-3264: (CCSS.API): Logical Charge Queue: Verify logicalChargeQueue API with invalid tenantID")
	public void CCSS_API_LogicalChargeQueueInValidTenant() {
		...
		//Set path
		String path = getPATH_LOGICALCHARGEQUEUE(generateUUID(), GARAGE_ID);
		...
		//Validate the Response
		sAssert.assertEquals(status, 401, "Logical Charge Queue API - Status Code for Invalid tenant id");
		//TODO: To be updated with proper response after backend implementation
		sAssert.assertEquals(bool, true, "Logical Charge Queue API - Response Body is empty for Invalid tenant id");
		sAssert.assertAll();

	}


	@Test(priority = 2, description = "TOUC-3265: (CCSS.API): Logical Charge Queue: Verify logicalChargeQueue API with invalid authorization key")
	public void CCSS_API_LogicalChargeQueueInValidAuthorizationKey() {
		...
		//Add Headers
		request.header("Content-Type", "application/json");
		request.header("authorization", "gdjkgsgalaeyroiryseh");
		...
		//Validate the Response
		sAssert.assertEquals(status, 401, "Logical Charge Queue API - Status Code for invalid authorization key");
		sAssert.assertEquals(message, "Unauthorized", "Logical Charge Queue API - Response Message for invalid authorization key");
		sAssert.assertAll();

	}


	@Test(priority = 3, description = "TOUC-3266: (CCSS.API): Logical Charge Queue: Verify logicalChargeQueue API with expired authorization key")
	public void CCSS_API_LogicalChargeQueueExpiredAuthorizationKey() {
		...
		//Validate the Response
		sAssert.assertEquals(status, 401, "Logical Charge Queue API - Status Code for expired authorization token");
		sAssert.assertEquals(message, "The incoming token has expired", "Logical Charge Queue API - Response Message for expired authorization token");
		sAssert.assertAll();
