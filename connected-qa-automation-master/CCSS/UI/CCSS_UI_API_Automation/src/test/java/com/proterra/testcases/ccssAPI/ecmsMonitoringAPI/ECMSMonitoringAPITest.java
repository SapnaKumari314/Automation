package test.java.com.proterra.testcases.ccssAPI.ecmsMonitoringAPI;
...
public class ECMSMonitoringAPITest extends APIBaseTest implements Modules{

	//Initiate Logger
	public static Logger log = Logger.getLogger(ECMSMonitoringAPITest.class.getName());
	...
	@Test(priority = 0, description = "TOUC-3280: (CCSS.API): ECMS Monitoring: Verify ECMS monitoring API with valid data and response")
	public void CCSS_API_ECMSMonitoringValidData() {
		...
		//Set path
		String path = getPATH_ECMSMONITORING(TENANT_ID, GARAGE_ID);
		...
		//Add Query Params
		request.queryParam("requestId", "ECMS_POWER_MONITORING-"+generateUUID());
		request.queryParam("start", Instant.now().minus(1,ChronoUnit.DAYS).truncatedTo(ChronoUnit.HOURS).toString());
		request.queryParam("end", Instant.now().truncatedTo(ChronoUnit.HOURS).toString());
		request.queryParam("aggIntervalInS", 3600);
		...
		sAssert.assertEquals(status, 200, "ECMS Monitoring API - Status Code for Valid user");
		sAssert.assertEquals(bool, false, "ECMS Monitoring API - Response Body is not empty for Valid user");
		sAssert.assertAll();
	...
	@Test(priority = 1, description = "TOUC-3281: (CCSS.API): ECMS Monitoring: Verify ECMS monitoring API with invalid tenantID")
	public void CCSS_API_ECMSMonitoringInValidTenant() {
		...
		//Set path
		String path = getPATH_ECMSMONITORING(generateUUID(), GARAGE_ID);
		...
		//Validate the Response
		sAssert.assertEquals(status, 401, "ECMS Monitoring API - Status Code for Invalid tenant id");
		//TODO: To be updated with proper response after backend implementation
		sAssert.assertEquals(bool, true, "ECMS Monitoring API - Response Body is empty for Invalid tenant id");
		sAssert.assertAll();

	}


	@Test(priority = 2, description = "TOUC-3282: (CCSS.API): ECMS Monitoring: Verify ECMS monitoring API with invalid authorization key")
	public void CCSS_API_ECMSMonitoringInValidAuthorizationKey() {
		...
		//Add Headers
		request.header("Content-Type", "application/json");
		request.header("authorization", "djgflaglagalghal");
		...
		//Validate the Response
		sAssert.assertEquals(status, 401, "ECMS Monitoring API - Status Code for invalid authorization key");
		sAssert.assertEquals(message, "Unauthorized", "ECMS Monitoring API - Response Message for invalid authorization key");
		sAssert.assertAll();

	}


	@Test(priority = 3, description = "TOUC-3283: (CCSS.API): ECMS Monitoring: Verify ECMS monitoring API with expired authorization key")
	public void CCSS_API_ECMSMonitoringExpiredAuthorizationKey() {
		...
		//Validate the Response
		sAssert.assertEquals(status, 401, "ECMS Monitoring API - Status Code for expired authorization token");
		sAssert.assertEquals(message, "The incoming token has expired", "ECMS Monitoring API - Response Message for expired authorization token");
		sAssert.assertAll();
