package test.java.com.proterra.testcases.ccssAPI.notificationsAPI;
...
public class NotificationsAPITest extends APIBaseTest implements Modules{

	//Initiate Logger
	public static Logger log = Logger.getLogger(NotificationsAPITest.class.getName());
	...
	@Test(priority = 0, description = "TOUC-3258: (CCSS.API): Notifications: Verify notifications API with valid data and response")
	public void CCSS_API_NotificationsValidData() {
		...
		//Set path
		String path = getPATH_NOTIFICATIONS(TENANT_ID);
		...
		//Add Query Params
		request.queryParam("trackSetupId", GARAGE_ID);
		request.queryParam("notificationState", "TRIGGERED");
		request.queryParam("requestId", "GET_NOTIFICATIONS-"+generateUUID());
		...
		sAssert.assertEquals(status, 200, "Notifications API - Status Code for Valid user");
		sAssert.assertEquals(bool, false, "Notifications API - Response Body is not empty for Valid user");
		sAssert.assertAll();
	...
	@Test(priority = 1, description = "TOUC-3259: (CCSS.API): Notifications: Verify notifications API with invalid tenantID")
	public void CCSS_API_NotificationsInValidTenant() {
		...
		//Set path
		String path = getPATH_NOTIFICATIONS(generateUUID());
		...
		//Validate the Response
		sAssert.assertEquals(status, 401, "Notifications API - Status Code for Invalid tenant id");
		//TODO: To be updated with proper response after backend implementation
		sAssert.assertEquals(bool, true, "Notifications API - Response Body is empty for Invalid tenant id");
		sAssert.assertAll();

	}


	@Test(priority = 2, description = "TOUC-3260: (CCSS.API): Notifications: Verify notifications API with invalid authorization key")
	public void CCSS_API_NotificationsInValidAuthorizationKey() {
		...
		//Add Headers
		request.header("Content-Type", "application/json");
		request.header("authorization", "sfgajkdgflajdgalg");
		...
		//Validate the Response
		sAssert.assertEquals(status, 401, "Notifications API - Status Code for invalid authorization key");
		sAssert.assertEquals(message, "Unauthorized", "Notifications API - Response Message for invalid authorization key");
		sAssert.assertAll();

	}


	@Test(priority = 3, description = "TOUC-3261: (CCSS.API): Notifications: Verify notifications API with expired authorization key")
	public void CCSS_API_NotificationsExpiredAuthorizationKey() {
		...
		//Validate the Response
		sAssert.assertEquals(status, 401, "Notifications API - Status Code for expired authorization token");
		sAssert.assertEquals(message, "The incoming token has expired", "Notifications API - Response Message for expired authorization token");
		sAssert.assertAll();
