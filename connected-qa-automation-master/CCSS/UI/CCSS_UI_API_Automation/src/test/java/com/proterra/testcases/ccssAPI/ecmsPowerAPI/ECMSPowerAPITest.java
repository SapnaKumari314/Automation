package test.java.com.proterra.testcases.ccssAPI.ecmsPowerAPI;
...
public class ECMSPowerAPITest extends APIBaseTest implements Modules{

	//Initiate Logger
	public static Logger log = Logger.getLogger(ECMSPowerAPITest.class.getName());
	...
	@Test(priority = 0, description = "TOUC-3285: (CCSS.API): ECMS Power: Verify ECMS power API with valid data and response")
	public void CCSS_API_ECMSPowerValidData() {
		...
		//Set path
		String path = getPATH_ECMSPOWER(TENANT_ID, GARAGE_ID);
		...
		//Add Query Params
		request.queryParam("requestId", "GET_ECMS_POWER_CAPACITY-"+generateUUID());
		...
		sAssert.assertEquals(status, 200, "ECMS Power API - Status Code for Valid user");
		sAssert.assertEquals(bool, false, "ECMS Power API - Response Body is not empty for Valid user");
		sAssert.assertAll();
	...
	@Test(priority = 1, description = "TOUC-3286: (CCSS.API): ECMS Power: Verify ECMS power API with invalid tenantID")
	public void CCSS_API_ECMSPowerInValidTenant() {
		...
		//Set path
		String path = getPATH_ECMSPOWER(generateUUID(), GARAGE_ID);
		...
		//Validate the Response
		sAssert.assertEquals(status, 401, "ECMS Power API - Status Code for Invalid tenant id");
		//TODO: To be updated with proper response after backend implementation
		sAssert.assertEquals(bool, true, "ECMS Power API - Response Body is empty for Invalid tenant id");
		sAssert.assertAll();

	}


	@Test(priority = 2, description = "TOUC-3287: (CCSS.API): ECMS Power: Verify ECMS power API with invalid authorization key")
	public void CCSS_API_ECMSPowerInValidAuthorizationKey() {
		...
		//Add Headers
		request.header("Content-Type", "application/json");
		request.header("authorization", "fjagljhadghadgkahdgdfgkj");
		...
		//Validate the Response
		sAssert.assertEquals(status, 401, "ECMS Power API - Status Code for invalid authorization key");
		sAssert.assertEquals(message, "Unauthorized", "ECMS Power API - Response Message for invalid authorization key");
		sAssert.assertAll();

	}


	@Test(priority = 3, description = "TOUC-3288: (CCSS.API): ECMS Power: Verify ECMS power API with expired authorization key")
	public void CCSS_API_ECMSPowerExpiredAuthorizationKey() {
		...
		//Validate the Response
		sAssert.assertEquals(status, 401, "ECMS Power API - Status Code for expired authorization token");
		sAssert.assertEquals(message, "The incoming token has expired", "ECMS Power API - Response Message for expired authorization token");
		sAssert.assertAll();
