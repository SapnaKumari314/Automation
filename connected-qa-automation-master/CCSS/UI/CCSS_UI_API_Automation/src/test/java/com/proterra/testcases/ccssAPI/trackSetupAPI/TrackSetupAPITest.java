package test.java.com.proterra.testcases.ccssAPI.trackSetupAPI;

import org.apache.log4j.Logger;
import org.testng.annotations.BeforeMethod;
import org.testng.annotations.Test;

import io.restassured.RestAssured;
import io.restassured.http.Method;
import io.restassured.response.Response;
import io.restassured.specification.RequestSpecification;
import main.java.com.proterra.AssertManager.SoftAssertLogger;
import test.java.com.proterra.testcases.ccssAPI.APIBaseTest;
import test.java.com.proterra.testcases.ccssAPI.Modules;
import test.java.com.proterra.testcases.ccssAPI.loadGaragesAPI.LoadGaragesAPITest;
import test.java.com.proterra.testcases.ccssAPI.loginAPI.LoginAPITest;

public class TrackSetupAPITest extends APIBaseTest implements Modules{

	//Initiate Logger
	public static Logger log = Logger.getLogger(TrackSetupAPITest.class.getName());
	public static SoftAssertLogger sAssert;
	public static LoginAPITest loginTest;
	public static LoadGaragesAPITest garageTest;



	/*-----------------------------------------------------Test Cases--------------------------------------------------------------*/


	@Test(priority = 0, description = "TOUC-3093: (CCSS.API): Tracks: Verify tracks API with valid data and response")
	public void CCSS_API_TrackSetupValidData() {

		//Assertions
		sAssert = new SoftAssertLogger();


		//Generate Access Token
		loginTest.generateAccessToken();

		//Get the Garage ID
		garageTest.getGarageIDs(GARAGE_NAME);

		//Set the module
		MODULE = Modules.MODULE_CCSS;

		//Set base
		String base = getBASE_PATH();

		//Set the Base URI
		RestAssured.baseURI = base;

		//Set path
		String path = getPATH_TRACKSETUP(TENANT_ID, GARAGE_ID);

		//Create the Request object
		RequestSpecification request = RestAssured.given();

		//Add Headers
		request.header("Content-Type", "application/json");
		request.header("authorization", ACCESS_TOKEN);

		//Add Query Params
		request.queryParam("requestId", "LOAD_TRACK_LIST_REQUEST_ID"+generateUUID());
		request.queryParam("tenantId", TENANT_ID);

		//Get Response
		Response response = request.request(Method.GET, path);
		int status = response.statusCode();

		String body = response.getBody().asString();
		boolean bool = false;
		if (body.isBlank() || body.isEmpty()) {
			bool = true;
		}

		//Validate the Response
		sAssert.assertEquals(status, 200, "Track Setup API - Valid user");
		sAssert.assertEquals(bool, false, "Track Setup API - Response Body is not empty for Valid user");
		sAssert.assertAll();


	}


	@Test(priority = 1, description = "TOUC-3094: (CCSS.API): Tracks: Verify tracks API with invalid tenantID")
	public void CCSS_API_TrackSetupInValidTenant() {

		//Assertions
		sAssert = new SoftAssertLogger();


		//Generate Access Token
		loginTest.generateAccessToken();

		//Get the Garage ID
		garageTest.getGarageIDs(GARAGE_NAME);

		//Set the module
		MODULE = Modules.MODULE_CCSS;

		//Set base
		String base = getBASE_PATH();

		//Set the Base URI
		RestAssured.baseURI = base;

		//Set path
		String path = getPATH_TRACKSETUP("90006981-51a0-48df-b31d-d66b2fb205e7", GARAGE_ID);

		//Create the Request object
		RequestSpecification request = RestAssured.given();

		//Add Headers
		request.header("Content-Type", "application/json");
		request.header("authorization", ACCESS_TOKEN);

		//Add Query Params
		request.queryParam("requestId", "LOAD_TRACK_LIST_REQUEST_ID"+generateUUID());
		request.queryParam("tenantId", "90006981-51a0-48df-b31d-d66b2fb205e7");

		//Get Response
		Response response = request.request(Method.GET, path);
		int status = response.statusCode();

		String body = response.getBody().asString();
		boolean bool = false;
		if (body.isBlank() || body.isEmpty() || body.equals("[]")) {
			bool = true;
		}

		//Validate the Response
		sAssert.assertEquals(status, 401, "Track Setup API - Status Code for Invalid tenant id");
		sAssert.assertEquals(bool, true, "Track Setup API - Response Body is empty for Invalid tenant id");
		sAssert.assertAll();

	}


	@Test(priority = 2, description = "TOUC-3095: (CCSS.API): Tracks: Verify tracks API with invalid authorization key")
	public void CCSS_API_TrackSetupInValidAuthorizationKey() {

		//Assertions
		sAssert = new SoftAssertLogger();


		//Generate Access Token
		loginTest.generateAccessToken();

		//Get the Garage ID
		garageTest.getGarageIDs(GARAGE_NAME);

		//Set the module
		MODULE = Modules.MODULE_CCSS;

		//Set base
		String base = getBASE_PATH();

		//Set the Base URI
		RestAssured.baseURI = base;

		//Set path
		String path = getPATH_TRACKSETUP(TENANT_ID, GARAGE_ID);

		//Create the Request object
		RequestSpecification request = RestAssured.given();

		//Add Headers
		request.header("Content-Type", "application/json");
		request.header("authorization", "shajfhaighaityoy");

		//Add Query Params
		request.queryParam("requestId", "LOAD_TRACK_LIST_REQUEST_ID"+generateUUID());
		request.queryParam("tenantId", TENANT_ID);

		//Get Response
		Response response = request.request(Method.GET, path);
		int status = response.statusCode();
		String message = responseToJson(response).getString("message");

		//Validate the Response
		sAssert.assertEquals(status, 401, "Track Setup API - Status Code with invalid authorization key");
		sAssert.assertEquals(message, "Unauthorized", "Track Setup API - Response Message with invalid authorization key");
		sAssert.assertAll();

	}


	@Test(priority = 3, description = "TOUC-3096: (CCSS.API): Tracks: Verify tracks API with expired authorization key")
	public void CCSS_API_TrackSetupExpiredAuthorizationKey() {

		//Assertions
		sAssert = new SoftAssertLogger();


		//Generate Access Token
		String expiredToken = "eyJraWQiOiJxdWxRc2RFWmw2ckZydGdCajExY0NLRktkdUUzVjRjWGRlS0N0TzBnbTJ3PSIsImFsZyI6IlJTMjU2In0..Dthe8sXsQpJkXPbJRz-pcTqdGklNCofIEIJYgwxT-3F3mJijaYmExouJ-QS2pUigQArzqIjw4SXOOm0AE64uaQm-pd2iyvmhl4gYs3ugHgkEVDEjeM7e85GvTOV2fMScHAdJ8sx9eubUglo_OFHBoE1XhtpDSmK6OqdyldRSZ599EOx10UsdM8QXapUPf_Nim9eZZ4IAiEzOURNyaO1_FvCR9xwse37BxK9uvl7qVzP_kx2InsnGlL9kpBq7Tbir3xTHLHpbGIc9KdWnYqJhnGvt3h93C8-cTMxBnzuzygzexJ7SiMOdci06RclDQveYNhazw_HKuWvcvbCJS1Z23g";

		//Get the Garage ID
		garageTest.getGarageIDs(GARAGE_NAME);

		//Set the module
		MODULE = Modules.MODULE_CCSS;

		//Set base
		String base = getBASE_PATH();

		//Set the Base URI
		RestAssured.baseURI = base;

		//Set path
		String path = getPATH_TRACKSETUP(TENANT_ID, GARAGE_ID);

		//Create the Request object
		RequestSpecification request = RestAssured.given();

		//Add Headers
		request.header("Content-Type", "application/json");
		request.header("authorization", expiredToken);

		//Add Query Params
		request.queryParam("requestId", "LOAD_TRACK_LIST_REQUEST_ID"+generateUUID());
		request.queryParam("tenantId", TENANT_ID);

		//Get Response
		Response response = request.request(Method.GET, path);
		int status = response.statusCode();
		String message = responseToJson(response).getString("message");

		//Validate the Response
		sAssert.assertEquals(status, 401, "Track Setup API - Status Code with expired authorization token");
		sAssert.assertEquals(message, "The incoming token has expired", "Track Setup API - Response Message with expired authorization token");
		sAssert.assertAll();

	}



	/*-----------------------------------------------------Setup and tear down--------------------------------------------------------------*/

	@BeforeMethod
	public void beforeTestCase() {

		loginTest = new LoginAPITest();
		garageTest =  new LoadGaragesAPITest();
		getTenantID();
	}

}
