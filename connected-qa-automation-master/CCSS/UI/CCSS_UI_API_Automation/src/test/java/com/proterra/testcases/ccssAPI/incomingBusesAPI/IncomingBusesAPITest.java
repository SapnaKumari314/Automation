package test.java.com.proterra.testcases.ccssAPI.incomingBusesAPI;
...
public class IncomingBusesAPITest extends APIBaseTest implements Modules{

	//Initiate Logger
	public static Logger log = Logger.getLogger(IncomingBusesAPITest.class.getName());
	...
	@Test(priority = 0, description = "TOUC-3239: (CCSS.API): Incoming Buses: Verify incomingBuses API with valid data and response")
	public void CCSS_API_IncomingBusesValidData() {
		...
		//Set path
		String path = getPATH_INCOMINGBUSES(TENANT_ID, GARAGE_ID);
		...
		request.queryParam("requestId", "GET_INCOMING_BUS_QUEUE-"+generateUUID());
		...
		sAssert.assertEquals(status, 200, "Incoming Buses API - Valid user");
		sAssert.assertEquals(bool, false, "Incoming Buses API - Response Body is not empty for Valid user");
		sAssert.assertAll();
	...
	@Test(priority = 1, description = "TOUC-3240: (CCSS.API): Incoming Buses: Verify incomingBuses API with invalid tenantID")
	public void CCSS_API_IncomingBusesInValidTenant() {
		...
		//Set path
		String path = getPATH_INCOMINGBUSES(generateUUID(), GARAGE_ID);
		...
		//Add Query Parameters
		request.param("requestId", "GET_INCOMING_BUS_QUEUE-"+generateUUID());
		request.param("size", "11");
		request.param("sortBy", "bookInTime");
		...
		//Validate the Response
		sAssert.assertEquals(status, 401, "Incoming Buses API - Status Code for Invalid tenant id");
		//TODO: To be updated with proper response after backend implementation
		sAssert.assertEquals(bool, true, "Incoming Buses API - Response Body is empty for Invalid tenant id");
		sAssert.assertAll();

	}


	@Test(priority = 2, description = "TOUC-3241: (CCSS.API): Incoming Buses: Verify incomingBuses API with invalid authorization key")
	public void CCSS_API_IncomingBusesInValidAuthorizationKey() {
		...
		//Validate the Response
		sAssert.assertEquals(status, 401, "Incoming Buses API - Status Code for invalid authorization key");
		sAssert.assertEquals(message, "Unauthorized", "Incoming Buses API - Response Message for invalid authorization key");
		sAssert.assertAll();

	}


	@Test(priority = 3, description = "TOUC-3242: (CCSS.API): Incoming Buses: Verify incomingBuses API with expired authorization key")
	public void CCSS_API_IncomingBusesExpiredAuthorizationKey() {
		...
		//Validate the Response
		sAssert.assertEquals(status, 401, "Incoming Buses API - Status Code for expired authorization token");
		sAssert.assertEquals(message, "The incoming token has expired", "Incoming Buses API - Response Message for expired authorization token");
		sAssert.assertAll();
